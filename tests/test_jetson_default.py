#!/usr/bin/env python3
"""
Jetson ST7789 Default Configuration Test
Tests the existing Jetson pinout (pins 29 and 31) before adding Waveshare support

This script:
1. Checks GPIO pin configuration
2. Tests SPI connectivity
3. Verifies display initialization
4. Runs basic display tests
5. Provides diagnostic information

Run with: sudo python3 test_jetson_default.py
"""
...
def check_root():
    ...
    if os.geteuid() != 0:
        ...
        print_info("Run: sudo python3 test_jetson_default.py")
        return False
...
def check_gpio_pins():
    """Check if pins 29 and 31 are configured as GPIO"""
...
def test_display_init():
    ...
    try:
        ...
        print_info("Initializing display (pins 29=DC, 31=RST)...")
        display = ST7789(
            spi_port=0,
            spi_cs=0,
            dc_pin=29,
...
def test_display_pattern(display):
    ...
    try:
        ...
        # Draw test text
        draw.text((10, 10), "Jetson Test", fill=(255, 255, 0))
        draw.text((10, 30), "Pin 29: DC", fill=(255, 255, 0))
        draw.text((10, 50), "Pin 31: RST", fill=(255, 255, 0))
...
║         Jetson ST7789 Default Configuration Test                 ║
║                                                                  ║
║  Testing Jetson default pinout before Waveshare migration       ║
║  Pins: DC=29, RST=31, SPI0, CS0                                 ║
╚══════════════════════════════════════════════════════════════════╝
""")
    ...
    if not gpio_ok:
        print_warning("\nGPIO pins may not be configured correctly")
        print_info("Continuing anyway - will fail if pins aren't ready...")
    ...
    if not display:
        ...
        print_info("  1. Check GPIO pins: sudo python3 tools/pin_inspector.py 29")
        print_info("  2. Check GPIO pins: sudo python3 tools/pin_inspector.py 31")
        print_info("  3. Verify physical connections")
        print_info("  4. Check device tree configuration")
        return False
    ...
    print_success("✓ Jetson default configuration verified")
...
║                    ✓ SUCCESS!                                    ║
║                                                                  ║
║  Jetson default configuration is working correctly.              ║
║  You can now proceed to set up Waveshare pinout.                ║
╚══════════════════════════════════════════════════════════════════╝
""")
