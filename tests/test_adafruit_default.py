#!/usr/bin/env python3
"""
Adafruit ST7789 Configuration Test
Tests the Adafruit pinout (pins 18 and 22)

This script:
1. Checks GPIO pin configuration for pins 18 and 22
2. Tests SPI connectivity
3. Verifies display initialization with Adafruit pins
...
Run with: sudo python3 test_adafruit_default.py
"""
...
def check_root():
    ...
    if os.geteuid() != 0:
        ...
        print_info("Run: sudo python3 test_adafruit_default.py")
        return False
...
def check_gpio_pins():
    """Check if pins 18 and 22 are configured as GPIO"""
    ...
    for pin in pins_to_check:
        ...
        if os.path.exists('tools/pin_inspector.py'):
            ...
            if result == 0:
                ...
                print_warning("You may need to install device tree overlay for Adafruit pins")
                all_ok = False
        else:
...
def test_display_init():
    """Test display initialization with Adafruit pins"""
    ...
    try:
        ...
        print_info("Initializing display with Adafruit pinout:")
        print_info("  DC  = Pin 22 (spi3_miso_py1)")
        print_info("  RST = Pin 18 (spi3_cs0_py3)")
        
        display = ST7789(
            spi_port=0,
            spi_cs=0,
            dc_pin=22,      # Adafruit DC pin
            rst_pin=18,     # Adafruit RST pin
            width=240,
            height=320,
            rotation=0,
...
def test_display_pattern(display):
    ...
    try:
        ...
        # Draw test text
        draw.text((10, 10), "Adafruit Test", fill=(255, 255, 0))
        draw.text((10, 30), "Pin 22: DC", fill=(255, 255, 0))
        draw.text((10, 50), "Pin 18: RST", fill=(255, 255, 0))
...
║       Adafruit ST7789 Configuration Test                         ║
║                                                                  ║
║  Testing Adafruit 2.0" 320x240 IPS TFT pinout on Jetson        ║
║  Pins: DC=22, RST=18, SPI0, CS0                                 ║
╚══════════════════════════════════════════════════════════════════╝
""")
    ...
    if not gpio_ok:
        ...
        print_info("\nAdafruit pins 18 and 22 need device tree configuration!")
        print_info("\nTo configure:")
        print_info("  1. Run: sudo ./compile_install_dtbo.sh jetson-st7789-adafruit.dts")
        print_info("  2. Add overlay to /boot/extlinux/extlinux.conf")
        print_info("  3. Reboot")
        print_info("  4. Run this test again")
        print_info("\nAlternatively, check individual pins:")
        print_info("  sudo python3 tools/pin_inspector.py 18")
    ...
    if not display:
        ...
        print_info("  1. Check GPIO pins: sudo python3 tools/pin_inspector.py 18")
    ...
    print_success("✓ Adafruit pinout verified")
...
║                    ✓ SUCCESS!                                    ║
║                                                                  ║
║  Adafruit pinout is working correctly.                           ║
║  You can now use: from_preset('adafruit')                        ║
╚══════════════════════════════════════════════════════════════════╝
""")
